...
const defaultSettings={font:'serif',size:19,line:1.7,theme:'system',paper:'gray',accent:'#D65C43',backgroundVeil:0.6,backgroundText:'dark',customBackgroundId:'',wpm:200,tts:1.0,autosave:true,showCode:false,markdownToolbar:true,headingCheck:true,headingMin:8,headingMax:80,sentenceCheck:true,sentenceMax:30,paragraphCheck:true,paragraphMax:650,frequentCheck:true,frequentMin:8,nearbyCheck:true,structureCheck:true,structureMax:1800,phraseCheck:true,openingCheck:true,headingStructureCheck:true,markdownCheck:true,aiStyleCheck:true,proofCheck:true,onlineSpelling:false,dzenCheck:true,dzenSmartRules:true,dzenCheckMode:'local',dzenAiBaseUrl:'https://openrouter.ai/api/v1',dzenAiModel:'openrouter/free',dzenAiSources:'https://dzen.ru/help/ru/requirements/rules.html',dzenAiStylePrompt:'Ищи шаблонные нейросетевые обороты, чрезмерно ровную структуру, искусственные противопоставления, повторяющиеся переходы и выводы, обезличенные обобщения, одинаковый ритм абзацев и другие признаки неестественного текста. Не пытайся определить авторство и не ставь процент ИИ. Отмечай только конкретные места и объясняй, что именно выглядит шаблонно.',riskCheck:true,riskWords:exampleRiskWords};
function normalizeDzenCheckMode(value){return value==='ai'||value==='both'?value:'local'}
function currentCheckMode(){return normalizeDzenCheckMode(settings&&settings.dzenCheckMode)}
function checkModeUsesLocal(){const mode=currentCheckMode();return mode==='local'||mode==='both'}
function checkModeUsesAi(){const mode=currentCheckMode();return mode==='ai'||mode==='both'}
