...
function saveUserSynonyms(){try{localStorage.setItem(USER_SYNONYMS_KEY,JSON.stringify(userSynonyms));updateUserSynonymStatus();return true}catch(e){toast('Не удалось сохранить накопленные синонимы');return false}}
function rememberUserSynonym(source,replacement){const key=String(source||'').trim().toLocaleLowerCase('ru-RU'),val=String(replacement||'').trim();if(!key||!val||key===val.toLocaleLowerCase('ru-RU'))return;const previous=Array.isArray(userSynonyms[key])?userSynonyms[key].slice():null;let arr=previous?previous.slice():[];arr=arr.filter(x=>String(x).toLocaleLowerCase('ru-RU')!==val.toLocaleLowerCase('ru-RU'));arr.unshift(val);userSynonyms[key]=arr.slice(0,20);if(!saveUserSynonyms()){if(previous)userSynonyms[key]=previous;else delete userSynonyms[key];updateUserSynonymStatus()}}
...
async function clearUserSynonyms(){const n=userSynonymPairCount();if(!n){toast('Накопленных синонимов пока нет');return}if(!await appConfirm('Удалить накопленные синонимы?',`Будут удалены все сохранённые пары: ${n}.`,'Удалить',true))return;const previous=userSynonyms;try{localStorage.removeItem(USER_SYNONYMS_KEY);userSynonyms={}}catch(e){userSynonyms=previous;toast('Не удалось очистить накопленные синонимы');return}updateUserSynonymStatus();if(replacementState)renderReplacement();toast('Накопленные синонимы удалены')}
